...
def insert_data(iplist):
    ...
    try:
        conn = MySQLdb.connect(host='127.0.0.1', user='root', passwd='DXjIR7^BDIDj%pkk', db='douban', charset='utf8')
        cursor = conn.cursor()
        cursor.execute('TRUNCATE TABLE douban_crawl_ip')
        cursor.executemany("INSERT INTO douban_crawl_ip(ip, port, lo, speed) VALUES(%s, %s, %s, %s)", iplist)
        conn.commit()
        cursor.close()
        conn.close()
